...
const CanvasInner: React.FC<CanvasProps> = ({
  ...
  return (
    <div
      ...
      className="w-full h-full relative select-none outline-none"
      onKeyDown={onKeyDown}
      tabIndex={0}
    >
      <ReactFlow
        ...
        onNodesChange={(changes: NodeChange[]) => {
          // Map position updates back to parent state
      ...
      >
        ...
        <Controls className="!m-4 !bg-slate-900 !border-slate-800 !text-slate-300" />
      </ReactFlow>

      {/* Empty State Overlay when no nodes exist */}
      {nodes.length === 0 && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/80 backdrop-blur-sm pointer-events-none z-10">
          <div className="bg-slate-900 p-8 rounded-2xl border border-slate-800 text-center max-w-md shadow-2xl pointer-events-auto space-y-4">
            <div className="w-12 h-12 bg-indigo-950/60 border border-indigo-900/60 rounded-xl flex items-center justify-center mx-auto text-indigo-400">
              <Sparkles className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-slate-100">Your story starts here</h3>
              <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                Create your first node to begin building your visual novel or dialogue flow. Press{' '}
                <kbd className="px-1.5 py-0.5 bg-slate-950 border border-slate-800 rounded text-[10px] text-slate-300 font-mono">
                  Space
                </kbd>{' '}
            ...
            <div className="flex flex-wrap gap-2 justify-center pt-2">
              <button
                ...
                className="px-3.5 py-2 bg-emerald-950/60 hover:bg-emerald-900/60 text-emerald-300 text-xs font-semibold rounded-lg border border-emerald-900/60 flex items-center gap-1.5 transition-colors"
              >
                <Plus className="w-4 h-4" /> + Narration
              </button>
              <button
                onClick={() => onAddNode('dialogue', { x: 300, y: 200 })}
                className="px-3.5 py-2 bg-indigo-950/60 hover:bg-indigo-900/60 text-indigo-300 text-xs font-semibold rounded-lg border border-indigo-900/60 flex items-center gap-1.5 transition-colors"
              >
