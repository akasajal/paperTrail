...
export const RightInspector: React.FC<RightInspectorProps> = ({
  ...
  // Render Nothing Selected / Project Info Overview
  if (!selectedNode && !selectedCharacter && !selectedLocation) {
    return (
      <div className="w-80 bg-slate-900 border-l border-slate-800 flex flex-col h-full z-20 select-none">
        ...
        <div className="p-5 flex flex-col items-center justify-center text-center text-slate-500 my-auto space-y-3">
          ...
          <div>
            <div className="text-sm font-semibold text-slate-300">No Element Selected</div>
            <p className="text-xs text-slate-500 mt-1 max-w-[200px]">
              Click any node on the canvas, or a character/location in the left sidebar to edit its properties.
            </p>
          </div>

          <div className="w-full pt-4 border-t border-slate-800 text-left space-y-2 text-xs text-slate-400">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-mono text-slate-200">{project.characters.length}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="w-80 bg-slate-900 border-l border-slate-800 flex flex-col h-full z-20 select-none">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800 bg-slate-950/40">
        <div className="flex items-center gap-2">
          ...
          {selectedNode.type === 'action' && (
            <span className="flex items-center gap-1.5 text-purple-400 text-xs font-semibold uppercase tracking-wider">
      ...
      <div className="p-4 space-y-4 overflow-y-auto flex-1 text-xs">
        ...
        <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Flag
              className={`w-4 h-4 ${isStartNode ? 'text-indigo-400 fill-indigo-400' : 'text-slate-500'}`}
            />
          ...
          {!isStartNode && (
            <button
              ...
              className="px-2.5 py-1 text-xs font-semibold bg-indigo-600/20 text-indigo-300 border border-indigo-500/40 hover:bg-indigo-600 hover:text-white rounded-md transition-all"
            >
              Make Start
            </button>
          )}
          ...
          <>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-slate-100 focus:outline-none focus:border-indigo-500 resize-none font-sans"
              />
            </div>
          </>
        )}
        ...
        {selectedNode.type === 'narration' && (
          <div>
            ...
            <textarea
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-slate-100 focus:outline-none focus:border-emerald-500 resize-none font-sans"
            />
          </div>
        )}
        ...
        {selectedNode.type === 'action' && (
          <div>
            ...
            <textarea
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-slate-100 focus:outline-none focus:border-purple-500 resize-none font-sans"
            />
          </div>
        )}
          ...
          <>
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-slate-100 focus:outline-none focus:border-amber-500 font-sans"
              />
            </div>
            ...
            <div className="space-y-2 pt-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-[11px] font-semibold text-amber-400 hover:text-amber-300 bg-amber-950/60 border border-amber-800/50 px-2 py-0.5 rounded transition-colors"
                >
        ...
        <div className="pt-3 border-t border-slate-800">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2 bg-red-950/40 hover:bg-red-900/60 border border-red-800/60 text-red-300 rounded-lg text-xs font-semibold transition-all"
          >
